...
export function upscaleClusteredData(clusteredData, tempW, tempH, targetW, targetH) {
...
export function clusteredDataToCanvas(clusteredData, tempW, tempH, scale = 1) {
	...
	const imgData = upscaleClusteredData(clusteredData, tempW, tempH, canvas.width, canvas.height);
...
export function getUniqueColors(clusteredData) {
	...
	for (let i = 0; i < clusteredData.length; i += 4) {
		seen.add(
			`${clusteredData[ i ]},${clusteredData[ i + 1 ]},${clusteredData[ i + 2 ]},${clusteredData[ i + 3 ]}`
		);
	}
	return seen.size;
}
